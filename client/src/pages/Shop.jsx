...
const IconShoe = () => (
	<svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#6b6b5e" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round">
...
const Shop = () => {
	...
	useEffect(() => {
		...
		setFilter({ category: categoryParam, brand: brandArray, sort: sortParam, minPrice: minParam, maxPrice: maxParam });
		setPage(pageParam);
		setPriceRange({ min: minParam, max: maxParam });
	}, [searchParams]);

	useEffect(() => { fetchData(); }, [filter, page]); // eslint-disable-line
	...
	const fetchData = async () => {
		...
		try {
			const params = {
				page, limit: 9, sort: filter.sort,
				...(filter.category               && { category: filter.category }),
				...(filter.brand.length > 0       && { brand: filter.brand.join(",") }),
			...
			if (resProd.success)  { setProducts(resProd.result.products); setTotalPages(resProd.result.pages); }
	...
	const updateParams = (nf) => {
		...
		nf.brand?.length > 0     ? p.set("brand",    nf.brand.join(","))   : p.delete("brand");
	...
	const handleCategoryChange = (slug) => updateParams({ ...filter, category: filter.category === slug ? "" : slug });
	const handleBrandToggle    = (slug) => {
		const isSel = filter.brand.includes(slug);
		updateParams({ ...filter, brand: isSel ? filter.brand.filter(s => s !== slug) : [...filter.brand, slug] });
	};
	const handlePresetPrice    = (min, max) => updateParams({ ...filter, minPrice: min, maxPrice: max });
	const handlePageChange     = (np)  => { const p = new URLSearchParams(searchParams); p.set("page", np); setSearchParams(p); };
	const handleSortChange     = (e)   => updateParams({ ...filter, sort: e.target.value });
	const handleResetFilter    = ()    => setSearchParams({});

	useEffect(() => {
		const t = setTimeout(() => {
			if (String(priceRange.min) !== String(filter.minPrice) || String(priceRange.max) !== String(filter.maxPrice))
				updateParams({ ...filter, minPrice: priceRange.min, maxPrice: priceRange.max });
		}, 500);
	...
	const isRangeActive = (min, max) =>
		String(filter.minPrice) === String(min) && String(filter.maxPrice) === String(max);

	const hasActiveFilters = filter.category || filter.brand.length > 0 || filter.minPrice || filter.maxPrice;
		...
		<div className="space-y-7">
			...
			<div>
				...
				<ul className="space-y-0.5">
					{[{ slug: "", name: "Tất cả sản phẩm" }, ...categories].map(cat => {
					...
					{brands.map(brand => {
						...
						return (
							<label key={brand._id}
								...
								<div onClick={() => handleBrandToggle(brand.slug)}
									...
									<input type="checkbox" className="hidden" checked={checked} onChange={() => handleBrandToggle(brand.slug)} />
									{checked && <IconCheck />}
								</div>
								<span className={`text-sm transition-colors ${checked ? "text-[#c2784d] font-semibold" : "text-[#5a4a3a]"}`}>
									{brand.name}
								</span>
							</label>
						);
			...
			<div>
				...
				<div className="space-y-0.5 mb-4">
					{PRICE_RANGES.map(range => {
						...
						return (
							<label key={range.id}
								onClick={() => handlePresetPrice(range.min, range.max)}
								...
								<div className={`w-3.5 h-3.5 rounded-full border-2 flex items-center justify-center shrink-0 transition-all
									...
									{active && <div className="w-1.5 h-1.5 rounded-full bg-[#c2784d]" />}
								</div>
								<span className={`text-sm ${active ? "text-[#c2784d] font-semibold" : "text-[#5a4a3a]"}`}>
									{range.label}
								</span>
							</label>
						);
				...
				<div className="flex items-center gap-2 mt-2">
					<input name="min" type="number" value={priceRange.min} onChange={handleManualPriceChange}
						placeholder="Từ"
					...
					<input name="max" type="number" value={priceRange.max} onChange={handleManualPriceChange}
						placeholder="Đến"
						className="w-full bg-white border border-[#e0d5c8] rounded-lg px-3 py-2 text-sm text-[#3a2a1a] placeholder-[#c0b5a8] outline-none focus:border-[#c2784d] focus:ring-2 focus:ring-[#c2784d]/10 transition-all" />
				</div>
			</div>
			...
			<div>
				...
				<div className="space-y-0.5">
					{[
						...
						return (
							<div key={opt.value}
								onClick={() => handleSortChange({ target: { value: opt.value } })}
			...
			<div className="shop-root bg-white min-h-screen">
				...
				<div className="bg-linear-to-br from-[#fff8f3] via-[#fdf4ee] to-[#fef9f6] border-b border-[#f0e5d8] px-4 py-10 text-center relative overflow-hidden">
					<div className="absolute inset-0 opacity-30"
						style={{ backgroundImage: "radial-gradient(circle at 20% 50%, #f5d5be 0%, transparent 50%), radial-gradient(circle at 80% 50%, #fce8d4 0%, transparent 50%)" }} />
					<p className="text-[10px] tracking-[0.3em] uppercase text-[#c2784d] mb-2 relative">Bộ sưu tập</p>
				...
				<div className="max-w-7xl mx-auto px-4 py-8 flex gap-8 bg-[#fafaf9]">
					...
					<aside className="hidden lg:block w-64 shrink-0">
						<div className="bg-white rounded-2xl p-6 sticky top-24 shadow-md shadow-[#e8d5c0]/40 border border-[#f0e5d8]">
							<div className="flex items-center gap-2 mb-7 pb-5 border-b border-[#f0e5d8]">
								<span className="text-[#c2784d]"><IconFilter /></span>
								<span className="shop-title text-base text-[#2a1a0e]">Bộ lọc</span>
								{hasActiveFilters && (
									<span className="ml-auto bg-[#c2784d] text-white text-[10px] font-bold px-2 py-0.5 rounded-full">
										{[filter.category ? 1 : 0, ...filter.brand.map(() => 1), filter.minPrice || filter.maxPrice ? 1 : 0].reduce((a, b) => a + b, 0)}
									</span>
								)}
							</div>
							<SidebarContent />
					...
					{sidebarOpen && (
						<div className="lg:hidden fixed inset-0 z-50 flex">
							<div className="mobile-overlay flex-1 bg-black/30" onClick={() => setSidebarOpen(false)} />
							<div className="w-80 bg-white h-full overflow-y-auto p-6 shadow-2xl">
								<div className="flex items-center justify-between mb-6 pb-4 border-b border-[#f0e5d8]">
									<span className="shop-title text-lg text-[#2a1a0e]">Bộ lọc</span>
					...
					<main className="flex-1 min-w-0">
						...
						{hasActiveFilters && (
							<div className="mb-5 flex flex-wrap items-center gap-2">
								<span className="text-xs text-[#8a8070] mr-1 font-medium">Đang lọc:</span>
								...
								{filter.category && (
									<span className="tag-pill inline-flex items-center gap-1.5 bg-white border border-[#e0d5c8] text-[#5a4a3a] text-xs font-medium px-3 py-1.5 rounded-full shadow-sm">
										{categories.find(c => c._id === filter.category)?.name ||
											categories.find(c => c.slug === filter.category)?.name || "Danh mục"}
										<button onClick={() => handleCategoryChange(filter.category)}
								...
								{filter.brand.map(bSlug => (
									<span key={bSlug} className="tag-pill inline-flex items-center gap-1.5 bg-white border border-[#e0d5c8] text-[#5a4a3a] text-xs font-medium px-3 py-1.5 rounded-full shadow-sm">
										{brands.find(b => b.slug === bSlug)?.name || "Brand"}
										<button onClick={() => handleBrandToggle(bSlug)}
											className="text-[#a0957e] hover:text-[#c2784d] transition-colors ml-0.5">
											<IconX size={11} />
										</button>
									</span>
								...
								{(filter.minPrice || filter.maxPrice) && (
									<span className="tag-pill inline-flex items-center gap-1.5 bg-white border border-[#e0d5c8] text-[#5a4a3a] text-xs font-medium px-3 py-1.5 rounded-full shadow-sm">
										{filter.minPrice ? new Intl.NumberFormat("vi-VN", { notation: "compact" }).format(filter.minPrice) : "0"}
										{" – "}
										{filter.maxPrice ? new Intl.NumberFormat("vi-VN", { notation: "compact" }).format(filter.maxPrice) : "∞"}
										<button onClick={() => handlePresetPrice("", "")}
											className="text-[#a0957e] hover:text-[#c2784d] transition-colors ml-0.5">
											<IconX size={11} />
										</button>
									</span>
						...
						{loading ? (
							...
							<>
								...
								<p className="text-xs text-[#a08070] mb-5">
									Hiển thị <span className="text-[#3a2a1a] font-semibold">{products.length}</span> sản phẩm
								</p>
							...
							<div className="flex flex-col items-center justify-center py-24 bg-[#fffaf7] rounded-2xl shadow-sm border border-[#f0e5d8] text-center">
								...
								<h3 className="shop-title text-xl text-[#2a1a0e] mb-2">Không tìm thấy sản phẩm</h3>
								<p className="text-sm text-[#a08070] max-w-xs mb-6">
									Thử điều chỉnh bộ lọc để tìm kiếm nhiều kết quả hơn.
								</p>
								<button onClick={handleResetFilter}
