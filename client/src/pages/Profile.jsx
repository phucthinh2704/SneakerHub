...
// Import API
import { apiGetUserProfile, apiUpdateUserProfile, apiGetMyOrders } from "../api/user";
// Import Action Redux (Giả sử bạn có authSlice như bài trước)
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-6 flex items-center space-x-6">
          <div className="relative">
            ...
            {/* Nút giả upload avatar */}
            <button className="absolute bottom-0 right-0 bg-gray-900 text-white p-2 rounded-full hover:bg-orange-600 transition">
              <Camera size={16} />
            </button>
          </div>
          <div>
        ...
        <div className="flex flex-col md:flex-row gap-8">
            ...
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
              <button
                ...
                className={`w-full flex items-center space-x-3 px-6 py-4 transition ${
                  activeTab === "info" ? "bg-orange-50 text-orange-600 border-l-4 border-orange-600" : "text-gray-600 hover:bg-gray-50"
                }`}
              >
                <User size={20} />
              ...
              <button
                ...
                className={`w-full flex items-center space-x-3 px-6 py-4 transition ${
                  activeTab === "orders" ? "bg-orange-50 text-orange-600 border-l-4 border-orange-600" : "text-gray-600 hover:bg-gray-50"
                }`}
              >
                <Package size={20} />
              ...
              <button
                ...
                className={`w-full flex items-center space-x-3 px-6 py-4 transition ${
                  activeTab === "password" ? "bg-orange-50 text-orange-600 border-l-4 border-orange-600" : "text-gray-600 hover:bg-gray-50"
                }`}
              >
                <Lock size={20} />
          ...
          <main className="w-full md:w-3/4">
            <div className="bg-white rounded-xl shadow-sm p-6 md:p-8">
              {activeTab === "info" && <ProfileInfo user={userData} dispatch={dispatch} />}
...
const ProfileInfo = ({ user, dispatch }) => {
  ...
  return (
    <div>
      <h2 className="text-xl font-bold mb-6 text-gray-800 border-b pb-4">Thông tin cá nhân</h2>
...
const ChangePassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await apiUpdateUserProfile({ password: passData.password });
      if (res.success) {
  ...
  return (
    <div>
      <h2 className="text-xl font-bold mb-6 text-gray-800 border-b pb-4">Đổi mật khẩu</h2>
...
const MyOrders = () => {
  ...
  if (loading) return <div className="py-10 text-center"><Loader2 className="animate-spin inline-block mr-2"/> Đang tải đơn hàng...</div>;
  ...
  return (
    <div>
      <h2 className="text-xl font-bold mb-6 text-gray-800 border-b pb-4">Lịch sử đơn hàng</h2>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="text-sm text-gray-500 border-b">
              ...
              <th className="py-3 font-medium text-right">Chi tiết</th>
            </tr>
          </thead>
          <tbody className="text-sm">
            {orders.map((order) => (
              <tr key={order._id} className="border-b last:border-0 hover:bg-gray-50 transition">
                <td className="py-4 font-mono text-gray-600">#{order._id.slice(-6).toUpperCase()}</td>
                <td className="py-4">{new Date(order.createdAt).toLocaleDateString("vi-VN")}</td>
                ...
                <td className="py-4 text-right">
                  <button className="text-orange-600 hover:text-orange-800 font-medium">Xem</button>
                </td>
              </tr>
            ))}
          </tbody>
